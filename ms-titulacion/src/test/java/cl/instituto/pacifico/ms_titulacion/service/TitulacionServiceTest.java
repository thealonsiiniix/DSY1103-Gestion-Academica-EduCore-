...
class TitulacionServiceTest {
    ...
    void listar_retornaLista() {
        List<Titulacion> lista = List.of(new Titulacion(1L, 10L, LocalDate.now(), "TITULADO"));
    ...
    void obtener_existente() {
        Titulacion titulacion = new Titulacion(1L, 10L, LocalDate.now(), "TITULADO");
        when(repository.findById(1L)).thenReturn(Optional.of(titulacion));
    ...
    void actualizar_existente() {
        Titulacion actual = new Titulacion(1L, 10L, LocalDate.of(2026,6,1), "PENDIENTE");
        Titulacion nueva = new Titulacion(null, 10L, LocalDate.of(2026,6,10), "TITULADO");
        when(repository.findById(1L)).thenReturn(Optional.of(actual));
        when(repository.save(any(Titulacion.class))).thenAnswer(inv -> inv.getArgument(0));
        Titulacion resultado = service.actualizar(1L, nueva);
        assertThat(resultado.getEstado()).isEqualTo("TITULADO");
        assertThat(resultado.getFecha()).isEqualTo(LocalDate.of(2026,6,10));
